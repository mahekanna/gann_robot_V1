...
"""
Index trading strategy using Gann Square of 9
Focuses on index options trading for both long and short signals
"""
...
class IndexStrategy(BaseStrategy):
    """
    Index trading strategy based on Gann Square of 9
    
    Trading rules:
    - Enter LONG with Call Options only
    - Enter SHORT with Put Options only
    - No spot market trades, only options
    """
    ...
    def __init__(self, 
        ...
        Args:
            ...
            option_lot_size: Standard lot size for options (e.g., 50 for NIFTY)
        """
        super().__init__(
        ...
        # Set up logger
        self.logger = logging.getLogger(f"index_strategy_{symbol}")
    ...
    def process_market_data(self, data: pd.DataFrame) -> SignalType:
        ...
        if self.is_call_active:  # Long position via call options
        ...
        elif self.is_put_active:  # Short position via put options
            # Check if stop loss is hit
            if current_price >= self.stoploss:
                self.logger.info(f"Stop loss hit at {current_price:.2f} (stop: {self.stoploss:.2f})")
    ...
    def calculate_option_size(self, capital: float, option_price: float, stop_loss: float) -> int:
        ...
        option_info = self.position_sizer.calculate_option_quantity(
            ...
            stop_loss=stop_loss,
            risk_adjustment=1.0,  # Standard risk for index options
            min_contracts=1,
    ...
    def handle_target_hit(self, target_index: int, current_price: float) -> Dict[str, Union[float, int]]:
        ...
        """
        # Calculate contracts to sell based on partial booking percentages
        if target_index >= len(self.partial_booking_percentages):
            target_index = len(self.partial_booking_percentages) - 1
        
        if self.is_call_active:
            contracts_to_sell = int(self.call_position * self.partial_booking_percentages[target_index])
            entry_price = self.call_entry_price
            self.logger.info(f"Call target {target_index+1} hit at {current_price:.2f}. Sell {contracts_to_sell} contracts.")
        elif self.is_put_active:
            contracts_to_sell = int(self.put_position * self.partial_booking_percentages[target_index])
            entry_price = self.put_entry_price
            self.logger.info(f"Put target {target_index+1} hit at {current_price:.2f}. Sell {contracts_to_sell} contracts.")
        else:
        ...
        if target_index == 0:
            ...
            # Initial trailing stop can vary based on option type
            if self.is_call_active:
                # For calls, move stop up to a certain percentage of profit
                self.stoploss = max(
                    self.stoploss,  # Original stop
                    entry_price + (current_price - entry_price) * 0.3  # 30% of current profit
                )
            elif self.is_put_active:
                # For puts, move stop down to a certain percentage of profit
                self.stoploss = min(
                    self.stoploss,  # Original stop
                    entry_price - (entry_price - current_price) * 0.3  # 30% of current profit
                )
        ...
        # For later targets, move stop to previous target
            ...
            if self.is_call_active:
                ...
                self.stoploss = min(self.stoploss, previous_target)
    ...
    def update_trailing_stop(self, current_price: float) -> Optional[float]:
        ...
        # Calculate potential new stop based on position type
        if self.is_call_active:
            # For long calls, we move the stop up as price increases
            if len(self.targets_hit) == 0:
                ...
                entry_price = self.call_entry_price
                potential_stop = max(entry_price, self.stoploss)
            else:
            ...
            # For long puts, we move the stop down as price decreases
            if len(self.targets_hit) == 0:
                # No targets hit yet, don't trail
                return None
            elif len(self.targets_hit) == 1:
                # First target hit, trail at breakeven or lower
                entry_price = self.put_entry_price
                potential_stop = min(entry_price, self.stoploss)
            else:
                # Multiple targets hit, trail at previous target
                last_hit_target_index = max(self.targets_hit)
                if last_hit_target_index > 0 and last_hit_target_index < len(self.active_targets):
                    previous_target = self.active_targets[last_hit_target_index - 1][1]
                    potential_stop = min(previous_target, self.stoploss)
                else:
                    potential_stop = self.stoploss
    ...
    def execute_long_strategy(self, 
                            current_price: float, 
                            capital: float, 
                            option_chain: pd.DataFrame) -> Dict[str, Any]:
        """
        Execute long strategy: Buy call options
        ...
        result = {
            ...
            "position_type": "CALL_OPTION",
            "orders": []
        }
        
        if option_chain is None or option_chain.empty:
            return {"error": "Option chain required for index strategy"}
        ...
        try:
            ...
            # Update state
            self.call_position = call_quantity
            self.call_entry_price = call_price
            self.call_symbol = call_symbol
            self.is_call_active = True
            
        except Exception as e:
            self.logger.error(f"Error setting up call option: {e}")
            return {"error": f"Failed to create long position: {e}"}
    ...
    def execute_short_strategy(self, 
                             ...
                             option_chain: pd.DataFrame) -> Dict[str, Any]:
        """
        Execute short strategy: Buy put options
    ...
    def execute_exit_long(self, 
        ...
        """
        Execute exit for long positions (call options)
        ...
        # Add call option exit order
        if "call_option" in position_details:
            call = position_details["call_option"]
            result["orders"].append({
                "symbol": call.get("symbol", self.call_symbol),
                "order_type": "MARKET",
                "transaction_type": "SELL",
                "quantity": call.get("quantity", self.call_position) * self.option_lot_size,
    ...
    def execute_exit_short(self, 
        ...
        if "put_option" in position_details:
            ...
            result["orders"].append({
                "symbol": put.get("symbol", self.put_symbol),
                "order_type": "MARKET",
                "transaction_type": "SELL",
                "quantity": put.get("quantity", self.put_position) * self.option_lot_size,
                "price": current_price,  # This will be market price
                "product_type": "MIS"  # Intraday
            })
    ...
    def calculate_position_size(self, capital: float, current_price: float, stop_loss: float) -> int:
        ...
        This is required by the base strategy, but for index strategy we use calculate_option_size instead
        ...
        """
        # For indices, we just return a reasonable minimum lot
        return 1