...
def load_data(symbol: str, 
    ...
    if not file_path.exists():
        ...
        if potential_files:
            ...
            from main import generate_equity_data
            ...
            df = generate_equity_data(
                ...
                volatility=0.015,
                filename=f"{symbol}_generated.csv"
            )
            ...
            # Save generated data
            file_path = Path(data_dir) / f"{symbol}_generated.csv"
    ...
    # Convert to specific timeframe if needed
    if timeframe != TimeFrame.MINUTE_1:
        from util.timeframe import TimeframeUtils
        df = TimeframeUtils.resample_ohlcv(df, timeframe)
